...
def single_stack_postorder_iterative_no_flags(root):
    ...
    marker = 1  # marker is a ghost node that is 'white'. If the ghost node is 'black', the marker will be removed by skip_siblings().
    stack = [root]
...
def single_stack_postorder_iterative_with_flags(root):
    ...
    stack = [root]
    marker = 0  # marker is a ghost node that tells the 'color' flag to invert
