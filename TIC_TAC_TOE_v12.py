...
class MainMenu:
    ...
    :ivar win_len: how many X/O in a row/column/diagonal to win.
   ...
   ██    ██  ██████          ██    ██   ██  ██████          ██     ██████  ███████''',
    ...
    def to_submenu(self, mode: int):
        ...
        for widget in self.window.winfo_children():
            widget.destroy()
        SubMenu(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, mode, self.ai_type)
...
class SubMenu:
    """
    :ivar window, board_len, win_len, board_zoom, colors: same as MainMenu.
    :ivar mode: 0 = player versus pc; 1 = player versus player.
    """

    def __init__(self, window, board_len, win_len, board_zoom, colors: dict, mode: int, ai_type):
        self.window = window
    ...
    def to_gamemenu(self, mode: int):
        ...
        GameMenu(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, mode, self.ai_type)

    def to_gamemenu_t(self, mode: int):
        for widget in self.window.winfo_children():
            widget.destroy()

        GameMenuT(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, mode, self.ai_type)

    def to_gamemenu_v(self, mode: int):
        for widget in self.window.winfo_children():
            widget.destroy()

        GameMenuV(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, mode, self.ai_type)

    def to_gamemenu_s(self, mode: int):
        for widget in self.window.winfo_children():
            widget.destroy()

        GameMenuS(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, mode, self.ai_type)

    def to_mainmenu(self):
        for widget in self.window.winfo_children():
            widget.destroy()

        MainMenu(self.window, ttt.board_len, ttt.win_len, self.board_zoom.get(), self.colors)

    def to_settings(self):
        for widget in self.window.winfo_children():
            widget.destroy()
        ColMenu(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, self.mode, self.ai_type)


class ColMenu:
    """
    :ivar window, board_len, win_len, board_zoom, colors, mode: same as SubMenu.
    ...
    def to_submenu(self):
        ...
        if messagebox.showinfo('Settings', f'Your settings have been updated!\n\n{self.colors}'):
            ...
            SubMenu(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, self.mode, self.ai_type)
...
class GameMenu:
    ...
    def __init__(self, window, board_len, win_len, board_zoom, colors: dict, mode: int, ai_type):
        self.window = window
        self.window.bind('<Configure>', self.update_scrollbars)  # update_scrollbars when window is resized
        self.window.bind('<MouseWheel>', self.mouse_scroll)  # scroll canvas vertically when any part of window has mouse wheel input
        self.window.bind('<Shift-MouseWheel>', self.shift_mouse_scroll)  # scroll canvas horizontally when any part of window has mouse wheel input
        ...
        self.title_frame = tk.Frame(self.settings_frame, background=self.colors[0]['background'])
        ...
        self.board_len_slider = tk.Scale(
            ...
            command=lambda val: self.update_len(int(val))  # 'val' is automatically passed by slider when it changes and is a str
        )
        ...
        self.board_zoom_slider = tk.Scale(
            ...
            to=13,
            cursor='sb_h_double_arrow',
            command=lambda val: self.update_zoom(int(val))  # 'val' is automatically passed by slider when it changes and is a str
        )
        ...
        self.settings_frame.pack(side='left', expand=False, fill='y', padx=(0, 5))
        self.settings_frame.grid_columnconfigure(0, minsize=50)
        self.settings_frame.grid_rowconfigure(11, weight=1)  # ensures debugger's row (row 11) can expand
        self.title_frame.grid(row=0, column=0, columnspan=3, pady=(0, 16), sticky='w')
        ...
        self.win_len_label.grid(row=5, column=1, sticky='e')
    ...
    def mouse_scroll(self, event):
        ...
        self.board_canvas.yview_scroll(-1 * (event.delta // 120), 'units')

    def shift_mouse_scroll(self, event):
        """Scroll horizontally with Shift + mouse wheel."""
        self.board_canvas.xview_scroll(-1 * (event.delta // 120), 'units')
    ...
    def update_buttonframe(self):
        ...
        for ind, button in enumerate(self.board_buttons):
            button.grid(row=ind // ttt.board_len, column=ind % ttt.board_len)  # reposition button
    ...
    def update_zoom(self, val: int):
        # update the position of the turn indicator and the scale of the buttons
        font_style = ('Helvetica', val * 4, 'bold' if val > 4 else 'normal')  # if font was bold when board_zoom == 4: button is not square

        for button in self.board_buttons:
            button.config(font=font_style)

        self.turn_hint[1].config(font=('Helvetica', val * 2, 'bold'))
        self.turn_hint[2].config(font=('Helvetica', val * 2, 'bold'))

    def lock_settings(self):
        self.board_len_slider.config(state='disabled')
        self.board_len_label.config(state='disabled')
        self.win_len_slider.config(state='disabled')
        self.win_len_label.config(state='disabled')
    ...
    def update_ind_pc(self):
        ...
        if self.filled_inds:  # if PC starts second or pre-filled board is loaded
            ...
            pc_move = random.randint(0, ttt.board_len ** 2 - 1)
            self.debugger.insert('end', f'Randomized ')
    ...
    def check_winner_pvc(self, plyr_or_pc: int) -> bool:
        """
        ...
        if formation is not None:
            ...
            if len(self.filled_inds) == ttt.board_len ** 2:  # if no one win and the whole board is filled
                ...
                self.turn_hint[plyr_or_pc].config(foreground='SystemDisabledText', background=self.colors[0]['background'], relief='flat')
                self.turn_hint[ttt.opp(plyr_or_pc)].config(foreground=self.colors[ttt.opp(plyr_or_pc)]['symbol'], background=self.colors[0]['foreground'], relief='ridge')
                return False
    ...
    def stop_game(self):
        self.filled_inds = []  # signal endgame
        for button in self.board_buttons:
    ...
    def replay(self):
        if messagebox.askyesno('Confirmation',
            ...
            GameMenu(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, self.mode, self.ai_type)


class GameMenuT(GameMenu):
    """
    :ivar window, board_len, win_len, board_zoom, colors, mode: same as SubMenu.
    :ivar remain_time: dict containing how much time does each player still have. Index 1 = X's remain_time; index 2 = O's remain_time
    :ivar hint_scale: the inflation of the active timer. Resets to 0 at the beginning of each turn.
    """

    def __init__(self, window, board_len, win_len, board_zoom, colors: dict, mode: int, ai_type):
        super().__init__(window, board_len, win_len, board_zoom, colors, mode, ai_type)
        self.remain_time = [0,
    ...
    def update_zoom(self, val: int):
        super().update_zoom(val)

        self.time_entry[1].config(font=('Courier', self.board_zoom.get() * 3 + 2, 'bold'))
        self.time_entry[2].config(font=('Courier', self.board_zoom.get() * 3 + 2, 'bold'))
    ...
    def validate_timer(self, plyr: int, *args):
        try:
            ...
            self.remain_time[plyr].set(10.0)

    def countdown(self):
        remain_time = self.remain_time[self.plyr].get()

        if remain_time > 0.0:
            # animate inflate of the current plyr's timer
            self.hint_scale = min(self.hint_scale + 2, 5)  # max scale should be odd number for ease-out
            ...
            self.remain_time[self.plyr].set(round(remain_time - 0.1, 1))
    ...
    def check_winner_pvc(self, plyr_or_pc: int) -> bool:
        ...
        if super().check_winner_pvc(plyr_or_pc) is False:
            # disable and reset font size of current plyr's timer
            self.time_entry[plyr_or_pc].config(relief='flat', disabledforeground='SystemDisabledText',
                                               disabledbackground=self.colors[0]['foreground'],
                                               font=('Courier', self.board_zoom.get() * 3 + 2, 'bold'))
            # enable next plyr's timer
            self.time_entry[ttt.opp(plyr_or_pc)].config(relief='sunken', disabledforeground=self.colors[ttt.opp(plyr_or_pc)]['symbol'], disabledbackground='white')

            self.remain_time[plyr_or_pc].set(self.remain_time[plyr_or_pc].get() + 1)
            return False
    ...
    def check_winner_pvp(self) -> bool:
        ...
        if super().check_winner_pvp() is False:  # changes self.plyr to next plyr
            ...
            self.remain_time[ttt.opp(self.plyr)].set(self.remain_time[ttt.opp(self.plyr)].get() + 1)
            return False
    ...
    def replay(self):
        if messagebox.askyesno('Confirmation',
            ...
            GameMenuT(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, self.mode, self.ai_type)
...
class GameMenuV(GameMenu):
    ...
    def update_len(self, val: int):
        super().update_len(val)
        self.remain_count_slider.config(from_=ttt.win_len, to=ttt.win_len * 2)

    def check_winner_pvc(self, plyr_or_pc: int) -> bool:
        self.del_moves()
        return super().check_winner_pvc(plyr_or_pc)
    ...
    def replay(self):
        if messagebox.askyesno('Confirmation',
            ...
            GameMenuV(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, self.mode, self.ai_type)


class GameMenuS(GameMenu):
    """
    :ivar window, board_len, win_len, board_zoom, colors, mode: same as SubMenu.
    :ivar prev_inputs: dict containing 2 lists: one containing the coordinates (x, y) of all previous moves made by X in chronological order, the other containing O's. Leftmost element = earliest move; rightmost element = latest move.
    """
    ...
    def update_ind_pc(self):
        pc_prev_inputs = self.prev_inputs[ttt.opp(self.plyr)]
        plyr_prev_inputs = self.prev_inputs[self.plyr]

        if pc_prev_inputs:  # if PC placed before
            pc_move = ttt.snake_pc_input(ttt.opp(self.plyr), self.main_board, pc_prev_inputs[-1] // ttt.board_len, pc_prev_inputs[-1] % ttt.board_len, plyr_prev_inputs[-1] // ttt.board_len,
                                         plyr_prev_inputs[-1] % ttt.board_len, self.is_debugging.get())

        elif plyr_prev_inputs:  # if PC never placed before but plyr has
            # set pc_row, pc_col same as plyr_row, plyr_col
            pc_move = ttt.snake_pc_input(ttt.opp(self.plyr), self.main_board, plyr_prev_inputs[-1] // ttt.board_len,
                                         plyr_prev_inputs[-1] % ttt.board_len, plyr_prev_inputs[-1] // ttt.board_len,
                                         plyr_prev_inputs[-1] % ttt.board_len, self.is_debugging.get())
        ...
        self.board_buttons[pc_move].config(text=ttt.convert_symbol(ttt.opp(self.plyr)),
                                           disabledforeground=self.colors[ttt.opp(self.plyr)]['symbol'], background=self.colors[ttt.opp(self.plyr)]['snake_head'], state='disabled')
        if self.prev_inputs[ttt.opp(self.plyr)]:
            # change the previous head color to body color
            self.board_buttons[self.prev_inputs[ttt.opp(self.plyr)][-1]].config(background=self.colors[ttt.opp(self.plyr)]['snake_body'])

        self.filled_inds.append(pc_move)
        self.prev_inputs[ttt.opp(self.plyr)].append(pc_move)
        self.main_board += ttt.opp(self.plyr) * ttt.three_pow[pc_move]
    ...
    def switch_snake(self, plyr_or_pc):
        """
         1. Enable valid adjacent buttons around the previous input of opponent and disable all other buttons.
         2. Trace back to the last cell that has valid adjacent cells if the head cell is stuck
        """

        def get_adj(row: int, col: int) -> set:
            """
            1. Enable valid adjacent buttons around the previous input of opponent.
            2. Return set containing the valid adjacent indexes around the previous input of opponent.
            """
            absolute_adj = set()
            ...
            if not absolute_adj:  # empty absolute_adj means next player is stuck
                # recursion to go back to previous input
                self.prev_inputs[plyr_or_pc].pop()
                prev_input = self.prev_inputs[plyr_or_pc][-1]
                absolute_adj = get_adj(prev_input // ttt.board_len, prev_input % ttt.board_len)
        ...
        for ind in set(range(ttt.board_len ** 2)).difference(self.filled_inds, absolute_adj):  # disable buttons that are empty and not adjacent to input
            self.board_buttons[ind].config(state='disabled', relief='sunken')

    def check_winner_pvc(self, plyr_or_pc: int) -> bool:
        if super().check_winner_pvc(plyr_or_pc) is False:
            self.switch_snake(ttt.opp(plyr_or_pc))
            return False

    def check_winner_pvp(self) -> bool:
        if super().check_winner_pvp() is False:  # changes self.plyr to next player
            self.switch_snake(self.plyr)
            return False
    ...
    def replay(self):
        if messagebox.askyesno('Confirmation',
            ...
            GameMenuS(self.window, self.board_len, self.win_len, self.board_zoom, self.colors, self.mode, self.ai_type)
